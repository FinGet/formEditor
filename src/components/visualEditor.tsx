import './style/visualEditor.less';
...
export const VisualEditor = defineComponent({
  ...
  setup(props, ctx) {
    ...
    console.log(dataModel.value.container);
    const containerStyle = computed(() => ({
      backgroundImage: dataModel.value.container.background? `url(${dataModel.value.container.background})`:"url('https://tva1.sinaimg.cn/large/e6c9d24ely1h68iiaw7xmj20u01o0jyh.jpg')"
    }));
    /*计算选中与未选择的block数据  */
    ...
    // vuedragger 插槽
    const DraggerSlots = {
      item: ({element , index}: {element:VisualEditorBlockData, index: number }) => (
        <VisualEditorBlock
          config={props.config}
          block={element}
          key={index}
          ...
          {...{
            onMousedown: (e: MouseEvent) => focusHandler.block.onMousedown(e, element, index),
          }}
        />
      )
    };
    ...
    return () => (
      <>
        <div class="visual-editor visual-editor-not-preview">
          {/* 左侧组件 */}
          <div class="visual-editor-menu">
            {props.config.componentCategories.map(category => (
              <div>
                <div class="visual-editor-menu-category">{category.label}</div>
                <div class="visual-editor-menu-items">
                  {props.config.componentList.filter(com => com.category === category.key).map(component => (
          ...
          <div class="visual-editor-body">
            <div class="visual-editor-content"
              ...
              >
                <Draggable
                  v-model={dataModel.value.blocks}
                  class="list-group"
                  ghost-class="ghost"
                  item-key="id"
                  move={checkMove}
                  v-slots={DraggerSlots}
                >  
                </Draggable>
              </div>
            </div>
